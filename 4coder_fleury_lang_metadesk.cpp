        ...
        if(F4_Index_RequireTokenKind(ctx, TokenBaseKind_Identifier, &name, F4_Index_TokenSkipFlag_SkipWhitespace))
        {
            if(F4_Index_RequireToken(ctx, S8Lit(":"), F4_Index_TokenSkipFlag_SkipWhitespace))
            {
                F4_Index_MakeNote(ctx->app, ctx->file, 0, F4_Index_StringFromToken(ctx, name),
                                  F4_Index_NoteKind_Constant, 0, Ii64(name));
            }
        }
...
internal F4_LANGUAGE_LEXFULLINPUT(F4_MD_LexFullInput)
{
    F4_MD_LexerState *state = (F4_MD_LexerState *)state_ptr;
    i64 maxi = (i64)max;
    if(maxi > (i64)state->string.size)
    {
        maxi = (i64)state->string.size;
    }
    for(i64 i = 0; i < maxi;)
    {
        i64 start_i = i;
           ...
           state->string.str[i+1] == '/')
        {
            ...
            token.size += 1;
            for(i64 j = i+2; j < maxi && state->string.str[j] != '\n'; j += 1, token.size += 1);
                ...
                state->string.str[i+1] == '*')
        {
            Token token = { i, 1, TokenBaseKind_Comment, 0 };
            token.size += 1;
            for(i64 j = i+2; j+1 < maxi && !(state->string.str[j] == '*' && state->string.str[j+1] == '/'); j += 1, token.size += 1);
            token.size += 2;
            token_list_push(arena, list, &token);
            i += token.size;
        }
        ...
        else if(chr == ',' || chr == ';' || (chr == '-' && i+1 < maxi && state->string.str[i+1] == '>'))
        {
